...
function InitalLogin() {
  ...
  const handleGoogleLogin = () => {
    ...
    }, 2000); // Clear the error message after 2000 milliseconds (2 seconds)
    // Here, you can initiate the Google login process or any other logic.
  };
  ...
  const handleAppleLogin = () => {
    ...
    // Here, you can initiate the Apple login process or any other logic.
  };
  return (
        ...
        <div className="flex flex-col items-center">
          <div className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-6">
              <button onClick={handleGoogleLogin} className="flex items-center border border-grayBorderLight hover:bg-grayBorderLight/20 gap-3 w-[300px] rounded-3xl bg-white text-black h-[40px] justify-center text-sm font-medium">
                <GoogleLogo size={20} />
                fazer login com o Google
              </button>
              {googleErrorMessage && (
            <div className="text-red-500 mt-[-1rem] text-xs">{googleErrorMessage}</div>
          )}
              <button onClick={handleAppleLogin} className="flex border border-grayBorderLight hover:bg-grayBorderLight/20 items-center gap-3 w-[300px] rounded-3xl bg-white text-black h-[40px] justify-center font-bold">
                <AppleLogo size={20} weight="fill" />
                entrar com Apple
              </button>
              {appleErrorMessage && (
            <div className="text-red-500 mt-[-1rem] text-xs">{appleErrorMessage}</div>
          )}
            </div>
    ...
    <div className="text-red-500 mt-1 text-xs">{emailError}</div>
            ...
            <div className="flex flex-col gap-6">
              <button onClick={handleAdvanceClick} className="flex bg-black font-bold text-sm items-center gap-3 w-[300px] rounded-3xl text-white h-[36px] justify-center">
                avançar
              </button>
        ...
        <button className="absolute left-[10px] hover:bg-gray-800/20 p-[10px] rounded-full top-[10px] "><X color="#000" size={18}/></button>
      </motion.div>
    </div>
  );
}
